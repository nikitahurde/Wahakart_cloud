<?php
$qry = $conn->query("SELECT p.*,s.categories_name as categories FROM purchase_order_list_pack p inner join categories s on p.categories_id = s.id  where p.id = '{$_GET['id']}'");
if ($qry->num_rows > 0) {
...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h4 class="card-title">Packaging Details - <?php echo $po_code ?></h4>
    </div>
    <div class="card-body" id="print_out">
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="supplier" class="control-label text-info">Product Name</label>
            ...
            <div hidden class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="categories_id" class="control-label text-info">Volume</label>
                        <div><?php echo isset($volume) ? $volume : '' ?></div>
                    </div>
                </div>
            </div>
            <h4 class="text-info">Packing</h4>
            <table class="table table-striped table-bordered" id="list">
                ...
                <tbody>
                    <?php
                    $total = 0;
                    $qry = $conn->query("SELECT p.*,i.name,i.description FROM `po_items_pack` p inner join item_list_p i on p.item_id = i.id where p.po_id = '{$id}'");
    ...
    <div class="card-footer py-1 text-center">
        ...
        <a class="btn btn-flat btn-primary" href="<?php echo base_url . '/admin?page=purchase_order_pack/manage_po_pack&id=' . (isset($id) ? $id : '') ?>">Edit</a>
        <a class="btn btn-flat btn-dark" href="<?php echo base_url . '/admin?page=purchase_order_pack' ?>">Back To List</a>
    </div>
</div>
...
<script>
    $(function() {
        $('#print').click(function() {
            ...
            _head.find('title').text("Packaging Details - Print View")
