<?php
if (isset($_GET['id'])) {
    $qry = $conn->query("SELECT p.*,s.categories_name as categories FROM purchase_order_list_prod p inner join categories s on p.categories_id = s.id  where p.id = '{$_GET['id']}'");
    if ($qry->num_rows > 0) {
...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h4 class="card-title"><?php echo isset($id) ? "Product Order Details - " . $po_code : 'Create New Product Order' ?></h4>
    </div>
    <div class="card-body">
        <form action="" id="po-form">
            ...
            <div class="container-fluid">
                ...
                <hr>
                <fieldset>
                    <legend class="text-info">Final Product Form</legend>
                    <div class="row " style="align-items:left;">
                        <?php
                        $item_arr = array();
                        $cost_arr = array();
                        $item = $conn->query("SELECT * FROM `products_list` where status = 1 order by `name` asc");
                        ...
                        <div class="col-md-5">
                            <div class="form-group">
                                <label for="item_id" class="control-label">Product</label>
                        ...
                        <div class="col-md-5">
                            <div class="form-group">
                                <label for="barcode_no">MFC No</label>
                                <input type="text" name="barcode_no" id="barcode_no" class="form-control">
                            </div>
                        </div>
                ...
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <tbody>
                        ...
                        $total = 0;
                        if (isset($id)) :
                            $qry = $conn->query("SELECT p.*,i.name,i.description FROM `po_items_prod` p inner join products_list i on p.item_id = i.id where p.po_id = '{$id}'");
...
<script>
    ...
    $(function() {
        ...
        $('#item_id').select2({
            placeholder: "Please select Brand first",
            width: 'resolve',
        })
        ...
        $('#po-form').submit(function(e) {
            ...
            start_loader();
            $.ajax({
                url: _base_url_ + "classes/Master.php?f=save_pro",
                ...
                success: function(resp) {
                    if (resp.status == 'success') {
                        location.replace(_base_url_ + "admin/?page=purchase_order_prod/view_pro&id=" + resp.id);
