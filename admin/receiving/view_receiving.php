...
if ($qry->num_rows > 0) {
    ...
    if ($from_order == 1) {
        $po_qry = $conn->query("SELECT p.*,s.categories_name as categories FROM `purchase_order_list` p inner join `categories` s on p.categories_id = s.id where p.id= '{$form_id}' ");
        ...
        $qry = $conn->query("SELECT b.*,s.categories_name as categories,p.po_code FROM back_order_list b inner join categories s on b.categories_id = s.id inner join purchase_order_list p on b.po_id = p.id  where b.id = '{$form_id}'");
        if ($qry->num_rows > 0) {
            foreach ($qry->fetch_array() as $k => $v) {
                if ($k == 'id')
                    $k = 'bo_id';
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="supplier" class="control-label text-info">Supplier</label>
