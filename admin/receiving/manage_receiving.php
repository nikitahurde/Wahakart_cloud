...
if (isset($_GET['id'])) {
    ...
    if ($qry->num_rows > 0) {
        ...
        if ($from_order == 1) {
            $qry = $conn->query("SELECT p.*,s.categories_name as categories FROM purchase_order_list p inner join categories s on p.categories_id = s.id  where p.id = '{$form_id}'");
            if ($qry->num_rows > 0) {
                foreach ($qry->fetch_array() as $k => $v) {
                    if ($k == 'id')
                        $k = 'po_id';
            ...
            $qry = $conn->query("SELECT b.*,s.categories_name as categories,p.po_code FROM back_order_list b inner join categories s on b.categories_id = s.id inner join purchase_order_list p on b.po_id = p.id  where b.id = '{$_GET['bo_id']}'");
            if ($qry->num_rows > 0) {
                foreach ($qry->fetch_array() as $k => $v) {
                    if ($k == 'id')
                        $k = 'bo_id';
...
if (isset($_GET['po_id'])) {
    $qry = $conn->query("SELECT p.*,s.categories_name as categories FROM purchase_order_list p inner join categories s on p.categories_id = s.id  where p.id = '{$_GET['po_id']}'");
    if ($qry->num_rows > 0) {
        foreach ($qry->fetch_array() as $k => $v) {
            if ($k == 'id')
                $k = 'po_id';
                ...
                <div class="row">
                    ...
                    <div hidden class="col-md-6">
                        <div class="form-group">
                            <label for="categories_id" class="control-label text-info">categories</label>
