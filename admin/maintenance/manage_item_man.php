...
if (isset($_GET['id']) && $_GET['id'] > 0) {
	$qry = $conn->query("SELECT * from `item_list_man` where id = '{$_GET['id']}' ");
	if ($qry->num_rows > 0) {
...
<div class="container-fluid">
	<form action="" id="item-form">
		...
		<div class="form-group">
			<label for="image" class="control-label">Employee Image</label><br>
		...
		<div class="form-group">
			...
			<input name="description" id="description" class="form-control form no-resize" value=" <?php echo isset($description) ? $description : ''; ?>">
		</div>
		<div class="form-group">
			<label for="cost" class="control-label">Sallary</label>
...
<script>
	$(document).ready(function() {
		...
		$('#item-form').submit(function(e) {
			...
			start_loader();
			$.ajax({
				url: _base_url_ + "classes/Master.php?f=save_item_man",
