					...
					<tbody>
						...
						?>
							<tr>
								...
								<td class="text-center"><img src="<?php echo $row['profile'] ?>" class="img-avatar img-thumbnail p-0 border-2" alt="image"></td>
								<td><?php echo date("Y-m-d H:i", strtotime($row['date_created'])) ?></td>
								<td><?php echo $row['name'] ?></td>

								<td hidden><?php echo $row['prod_qty'] ?></td>
								<td><?php echo $row['cost'] ?></td>
								<?php
								$brand_id = $row['brands_id'];
								$cat = $conn->query("SELECT `brands_name` FROM `brands` WHERE id=$brand_id");
								while ($row2 = $cat->fetch_assoc()) :
								?>
									<td hidden><?php echo $row2['brands_name'] ?></td>
								<?php endwhile; ?>
			...
			<div class="modal-body">
				...
				<div class="myform">
					<form action="" method="POST" enctype="multipart/form-data">
						...
						<div class="form-group">
							<label hidden for="prod_qty" class="control-label">QTY</label>
							<input hidden type="number" name="prod_qty" id="prod_qty" step="any" class="form-control rounded-0 text-end" value="<?php echo isset($prod_qty) ? $prod_qty : ''; ?>">
						</div>

						<div class="form-group">
							<label hidden for="categories_id" class="control-label">Categories</label>
							<select hidden name="categories_id" id="categories_id" class="custom-select select2">
								<option disabled></option>
								<?php
								$categories = $conn->query("SELECT * FROM `categories` where categories_status = 1 order by `categories_name` asc");
								while ($row = $categories->fetch_assoc()) :
									$selected = "";
									if (isset($categories_id) && $categories_id == $row['id']) {
										$selected = "selected";
									} elseif (!isset($categories_id) && $row['id'] == $default_categories_id_from_database) {
										$selected = "selected";
									}
								?>
						...
						<div class="form-group">
							<label hidden for="brands_id" class="control-label">Brand</label>
							<select hidden name="brands_id" id="brands_id" class="custom-select select2">
								<option disabled></option>
								<?php
								$brands = $conn->query("SELECT * FROM `brands` WHERE brands_status = 1 ORDER BY `brands_name` ASC");
								while ($row = $brands->fetch_assoc()) {
									$selected = '';
									if (isset($brands_id) && $brands_id == $row['id']) {
										$selected = 'selected';
									} elseif (!isset($brands_id) && $row['brands_name'] == $default_category_name) {
										$selected = 'selected';
									}
								?>
...
if (isset($_POST['upload'])) {
	...
	$query = "INSERT INTO `products_list`(`name`,`description`,`profile`,`cost`,`prod_qty`,`categories_id`,`brands_id`) VALUES ('$name','$description','$img_path','$cost','$prod_qty','$categories_id','$brands_id')";
...
<script>
	$(document).ready(function() {
		$('.delete_data').click(function() {
			_conf("Are you sure to delete this Product permanently?", "delete_products", [$(this).attr('data-id')])
		})
		$('#create_new').click(function() {
			uni_modal("<i class='fa fa-plus'></i> Add New Product", "maintenance/manage_products.php", "mid-large")
		})
		$('.edit_data').click(function() {
			uni_modal("<i class='fa fa-edit'></i> Edit Product Details", "maintenance/manage_final_products.php?id=" + $(this).attr('data-id'), "mid-large")
		})
		$('.view_data').click(function() {
			uni_modal("<i class='fa fa-box'></i> Product Details", "maintenance/view_final_products.php?id=" + $(this).attr('data-id'), "")
		})
	...
	function delete_products($id) {
		start_loader();
		$.ajax({
			url: _base_url_ + "classes/Master.php?f=delete_products",
			method: "POST",
			data: {
				id: $id
			},
