...
<?php
$qry = $conn->query("SELECT i.*,s.categories_name as categories FROM `item_list` i inner join categories s on i.categories_id = s.id where  i.id = '{$_GET['id']}' ");
if ($qry->num_rows > 0) {
    foreach ($qry->fetch_assoc() as $k => $v) {
        $$k = $v;
    }
}
$qry2 = $conn->query("SELECT i.*,s.brands_name as brands FROM `item_list` i inner join brands s on i.brands_id = s.id where  i.id = '{$_GET['id']}' ");
if ($qry2->num_rows > 0) {
