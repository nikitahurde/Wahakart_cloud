<?php
if (isset($_GET['id'])) {
    $qry = $conn->query("SELECT p.* FROM receiving_list_man p where p.id = '{$_GET['id']}'");
    if ($qry->num_rows > 0) {
        foreach ($qry->fetch_array() as $k => $v) {
            $$k = $v;
        }
        if ($from_order == 1) {
            $qry = $conn->query("SELECT p.*,s.categories_name as categories FROM purchase_order_list_man p inner join categories_man s on p.categories_id = s.id  where p.id = '{$form_id}'");
            if ($qry->num_rows > 0) {
                foreach ($qry->fetch_array() as $k => $v) {
                    if ($k == 'id')
                        $k = 'po_id';
            ...
            $qry = $conn->query("SELECT b.*,s.categories_name as categories,p.po_code FROM back_order_list_man b inner join categories_man s on b.categories_id = s.id inner join purchase_order_list_man p on b.po_id = p.id  where b.id = '{$_GET['bo_id']}'");
            if ($qry->num_rows > 0) {
                foreach ($qry->fetch_array() as $k => $v) {
                    if ($k == 'id')
                        $k = 'bo_id';
...
if (isset($_GET['po_id'])) {
    $qry = $conn->query("SELECT p.*,s.categories_name as categories FROM purchase_order_list_man p inner join categories_man s on p.categories_id = s.id  where p.id = '{$_GET['po_id']}'");
    if ($qry->num_rows > 0) {
        foreach ($qry->fetch_array() as $k => $v) {
            if ($k == 'id')
                $k = 'po_id';
        ...
        <form action="" id="receive-form">
            ...
            <div class="container-fluid">
                <div class="row">
                    ...
                    <div hidden class="col-md-6">
                        <div class="form-group">
                            <label for="categories_id" class="control-label text-info">Employee type</label>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="categories_id" class="control-label text-info">Product</label>

                            <input type="text" class="form-control form-control-sm rounded-0" value="<?php echo isset($supplier) ? $supplier : '' ?>" readonly>
                        </div>
                    </div>
                </div>
                ...
                <div hidden class="row">
                    <div class="col-md-6">
                        <label class="control-label text-info">Barcode No</label>
                ...
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <thead>
                        <tr class="text-light bg-navy">
                            ...
                            <th class="text-center py-1 px-2">Worker</th>
                    ...
                    <tbody>
                        ...
                        if (isset($po_id)) :
                            if (!isset($bo_id))
                                $qry = $conn->query("SELECT p.*,i.name,i.description FROM `po_items_man` p inner join item_list_man i on p.item_id = i.id where p.po_id = '{$po_id}'");
                            else
                                $qry = $conn->query("SELECT b.*,i.name,i.description FROM `bo_items_man` b inner join item_list_man i on b.item_id = i.id where b.bo_id = '{$bo_id}'");
                            while ($row = $qry->fetch_assoc()) :
                                $total += $row['total'];
                                $row['qty'] = $row['quantity'];
                                if (isset($stock_ids)) {
                                    // echo "SELECT * FROM `stock_list` where id in ($stock_ids) and item_id = '{$row['item_id']}'";
                                    $qty = $conn->query("SELECT * FROM `stock_list_man` where id in ($stock_ids) and item_id = '{$row['item_id']}'");
...
<script>
    $(function() {
        ...
        $('#receive-form').submit(function(e) {
            ...
            start_loader();
            $.ajax({
                url: _base_url_ + "classes/Master.php?f=save_receiving_man",
                ...
                success: function(resp) {
                    if (resp.status == 'success') {
                        location.replace(_base_url_ + "admin/?page=receiving_man/view_receiving_man&id=" + resp.id);
