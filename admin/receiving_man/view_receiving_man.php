<?php
$qry = $conn->query("SELECT * FROM receiving_list_man where id = '{$_GET['id']}'");
if ($qry->num_rows > 0) {
    foreach ($qry->fetch_array() as $k => $v) {
        $$k = $v;
    }
    if ($from_order == 1) {
        $po_qry = $conn->query("SELECT p.*,s.categories_name as categories FROM `purchase_order_list_man` p inner join `categories_man` s on p.categories_id = s.id where p.id= '{$form_id}' ");
        ...
        $qry = $conn->query("SELECT b.*,s.categories_name as categories,p.po_code FROM back_order_list_man b inner join categories_man s on b.categories_id = s.id inner join purchase_order_list_man p on b.po_id = p.id  where b.id = '{$form_id}'");
        if ($qry->num_rows > 0) {
            foreach ($qry->fetch_array() as $k => $v) {
                if ($k == 'id')
                    $k = 'bo_id';
...
<div class="card card-outline card-primary">
    ...
    <div class="card-body" id="print_out">
        <div class="container-fluid">
            ...
            <h4 class="text-info">Manufacturing</h4>
            <table class="table table-striped table-bordered" id="list">
                ...
                <thead>
                    <tr class="text-light bg-navy">
                        <th class="text-center py-1 px-2">Workers</th>
                        <th class="text-center py-1 px-2">Work</th>
                        <th class="text-center py-1 px-2">O</th>
                ...
                <tbody>
                    <?php
                    $total = 0;
                    $qry = $conn->query("SELECT s.*,i.name,i.description FROM `stock_list_man` s inner join item_list_man i on s.item_id = i.id where s.id in ({$stock_ids})");
    ...
    <div class="card-footer py-1 text-center">
        ...
        <a class="btn btn-flat btn-primary" href="<?php echo base_url . '/admin?page=receiving_man/manage_receiving_man&id=' . (isset($id) ? $id : '') ?>">Edit</a>
        <a class="btn btn-flat btn-dark" href="<?php echo base_url . '/admin?page=receiving_man' ?>">Back To List</a>
    </div>
</div>
